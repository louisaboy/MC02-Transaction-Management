...
function recover () {
    ...
    if (isNode1Online) {
        db1.query("SELECT MAX(time_id) as max_id FROM recovery", function (err, result1) {
            timeId1 = result1[0].max_id;
            console.log("timeID1 query: ", timeId1);
        });
    }
    if (isNode2Online) {
        db2.query("SELECT MAX(time_id) as max_id FROM recovery", function (err, result2) {
            timeId2 = result2[0].max_id;
            console.log("timeID2 query: ", timeId2);
        });
    }
    if (isNode3Online) {
        db3.query("SELECT MAX(time_id) as max_id FROM recovery", function (err, result3) {
    ...
    setTimeout(function(){
        ...
        if (timeId1 < headTimeId && isNode1Online) {
            ...
            let sql = "SELECT * FROM recovery WHERE `time_id` > " + timeId1;
                ...
                for (var i = 0; i < headTimeId-timeId1; i++) {
                    ...
                    if (isNode1Online) {
                        try {
                            ...
                            let sql = "INSERT INTO recovery SET ?";
...
function insertToRecover(id, name, year, rank, action) {
    ...
    if (isNode1Online) {
        try {
            let sql = "SELECT MAX(time_id) as max_id FROM recovery";
    ...
    if (isNode2Online) {
        try {
            let sql2 = "SELECT MAX(time_id) as max_id FROM recovery";
    ...
    if (isNode3Online) {
        try {
            let sql3 = "SELECT MAX(time_id) as max_id FROM recovery"
